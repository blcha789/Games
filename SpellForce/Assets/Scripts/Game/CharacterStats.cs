...
public class CharacterStats : MonoBehaviour
{
    ...
    private void Regeneration()
    {
        ...
        for (int i = 0; i < mana.Length; i++)
        {
            ...
            {
                mana[i].currentMana += mana[i].manaRegen * Time.deltaTime;
                //UI
            }
        }
    }
    ...
    public void TakeMana(ManaType manaType, float manaAmount)
    {
        for (int i = 0; i < mana.Length; i++)
        {
            if (mana[i].name == manaType)
            {
                mana[i].currentMana -= manaAmount;
                //UI
            }
        }
    }
